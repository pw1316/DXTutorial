...
class System {
 public:
  ...
  void SendMessageCommand(const std::string& message, ULONG body) {
    Core::MessageHandler().ExecuteCommand(Core::Message(message, body));
  }
  void SendMessageCommand(const std::string& message) {
    Core::MessageHandler().ExecuteCommand(Core::Message(message, 0));
  }
  LRESULT CALLBACK SysMessageHandler(HWND hWnd, UINT message, WPARAM wParam,
                                     LPARAM lParam) {
    switch (message) {
      case WM_KEYDOWN: {
 ...
 private:
  static LRESULT CALLBACK WinProc(HWND hWnd, UINT message, WPARAM wParam,
    ...
    switch (message) {
      case WM_COMMAND: {
        ...
        switch (wmId) {
          case IDM_ABOUT:
            DialogBox(g_hInst, MAKEINTRESOURCE(IDD_NAIIVE_ABOUT), hWnd, About);
            break;
          case IDM_EXIT:
            PostQuitMessage(0);
            break;
  ...
  void InitializeWindow() {
    ...
    wcex.hIconSm =
        LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_NAIIVE_ICON_SMALL));
    hr = (RegisterClassEx(&wcex) == 0) ? E_FAIL : S_OK;
    FAILTHROW;

    RECT paintRect{0, 0, 1024, 768};
    hr =
        AdjustWindowRect(&paintRect, WS_OVERLAPPEDWINDOW, true) ? S_OK : E_FAIL;
    FAILTHROW;
    m_hwnd = CreateWindow(m_className, m_appName, WS_OVERLAPPEDWINDOW,
